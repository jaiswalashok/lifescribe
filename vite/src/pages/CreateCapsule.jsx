...
export default function CreateCapsule() {
  ...
  const [addMode, setAddMode] = useState('individual'); // 'individual' | 'circle'
  ...
  const filteredConnections = searchQ
    ? connections.filter(c =>
        c.connected_user_name?.toLowerCase().includes(searchQ.toLowerCase()) &&
        !invitees.find(inv => inv.id === c.id)
      )
    : connections.filter(c => !invitees.find(inv => inv.id === c.id));

  const addInvitee = (conn) => {
    setInvitees(prev => prev.find(i => i.id === conn.id) ? prev : [...prev, conn]);
    setSearchQ('');
  };
  ...
  const addCircle = (circle) => {
    ...
    members.forEach(m => {
      setInvitees(prev => prev.find(i => i.id === m.id) ? prev : [...prev, m]);
    });
  };
  ...
  const handleSave = async () => {
    ...
    await base44.entities.MomentCapsule.create({
      title,
      event_date: eventDate,
      description: description || undefined,
      invite_link_token: token,
      contributor_count: invitees.length,
    });
  ...
  const copyLink = () => {
    const link = `${window.location.origin}/capsule/join/${createdToken}`;
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-8 text-center">
        ...
        <div className="w-full bg-[#F5F5F5] rounded-xl p-4 mb-4 text-left">
          ...
          <p className="text-xs text-[#111111] break-all font-mono">
            {window.location.origin}/capsule/join/{createdToken}
          </p>
        </div>

        <Button
          onClick={copyLink}
          className="w-full bg-[#111111] text-white hover:bg-[#333] rounded-full h-12 text-sm font-medium mb-3"
        >
      ...
      <div className="px-6 space-y-5">
        <div>
          ...
          <Input
            ...
            placeholder="e.g. Farid's Wedding, Bali 2024…"
        ...
        <div>
          ...
          <Textarea
            ...
            placeholder="Add some context about this shared memory…"
        ...
        <div>
          ...
          <div className="flex gap-2 mb-3">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                addMode === 'individual' ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-gray-500'
              }`}
            >
              <Users className="w-3 h-3" /> Individual
            </button>
            <button
              onClick={() => setAddMode('circle')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                addMode === 'circle' ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-gray-500'
              }`}
            >
              Add by Circle
            </button>
          ...
          {addMode === 'individual' && (
            <>
              <Input
                ...
                className="bg-[#F5F5F5] border-0 h-10 rounded-xl text-sm text-[#111111] placeholder:text-gray-300 mb-1"
              />
              <div className="max-h-44 overflow-y-auto rounded-xl bg-white border border-gray-100 shadow-sm">
                {filteredConnections.slice(0, 8).map(conn => (
                  <button
                    key={conn.id}
                    onClick={() => addInvitee(conn)}
                    className="w-full flex items-center gap-2 p-2.5 hover:bg-gray-50 text-left"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {conn.relationship_label && (
                        <p className="text-[10px] text-gray-400">{conn.relationship_label}</p>
                      )}
                    </div>
          ...
          {addMode === 'circle' && (
            <div className="space-y-2">
              {circles.length === 0 && (
                <p className="text-xs text-gray-400">No circles yet.</p>
              )}
              {circles.map(circle => {
                const members = connections.filter(c => c.circle_id === circle.id);
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 rounded-xl bg-[#F5F5F5] hover:bg-gray-100 transition-all text-left"
                  >
                    <div>
                      <p className="text-sm font-semibold text-[#111111]">{circle.name}</p>
                      <p className="text-xs text-gray-400">{members.length} members</p>
                    </div>
                    <div className="flex -space-x-1.5">
                      {members.slice(0, 3).map((m, i) => (
                        <MoodRingAvatar key={i} src={m.connected_user_avatar} mood={m.connected_user_mood} size={22} name={m.connected_user_name} />
                      ))}
                    </div>
                  </button>
                );
              ...
              <p className="text-xs text-gray-400 pt-1">
                You can also share an invite link after creating — anyone with the link can join.
              </p>
            </div>
          )}
        </div>
