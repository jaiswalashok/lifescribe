...
export default function EditProfile() {
  ...
  const section = urlParams.get('section') || 'about me';
  ...
  useEffect(() => {
    if (profiles.length > 0) {
      setForm(profiles[0]);
      // Extract country from current_location if it matches a saved value
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      handleChange('profile_picture_url', file_url);
  ...
  const handleSave = async () => {
    ...
    if (profiles.length > 0) {
      ...
      // Ensure current_location has a value if city is selected
      if (form.current_location === undefined || form.current_location === null || form.current_location === '') {
        if (!selectedCountry) {
          setToast('Please select both country and city.');
          setSaving(false);
          return;
        }
      }
      await base44.entities.UserProfile.update(profiles[0].id, dataToSave);
    }
  ...
  const title = section === 'about me' ? 'About Me' : section === 'location' ? 'Location' :
    section === 'relationship' ? 'Relationship Status' : 'Work';
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-6 space-y-5">
           ...
           <>
             ...
             <div>
               ...
               <Select value={form.current_location || ''} onValueChange={v => handleChange('current_location', v)} disabled={!selectedCountry}>
                 ...
                 <SelectContent>
                   {getCurrentCitiesList().map(city => <SelectItem key={city} value={city}>{city}</SelectItem>)}
                 </SelectContent>
               </Select>
             </div>
             <div>
        ...
        <Button
          onClick={handleSave}
          disabled={saving}
          className="w-full bg-[#111111] text-white hover:bg-[#333] rounded-full h-12 text-base font-medium disabled:opacity-40 mt-4"
        >
          {saving ? 'Saving...' : 'Save'}
        </Button>
      </div>
