...
export default function CreateAccount() {
  ...
  const handleSubmit = async () => {
    ...
    navigate(createPageUrl('EmailVerify') + `?email=${encodeURIComponent(form.email)}`);
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col px-6 pt-6 pb-8">
      ...
      <p className="text-center text-sm text-gray-400 mt-4">
        ...
        <button onClick={() => navigate(createPageUrl('Home'))} className="text-[#1A1A2E] font-medium underline">
          Sign in
        </button>
      </p>
    </div>
