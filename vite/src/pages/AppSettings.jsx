...
export default function AppSettings() {
  ...
  const planLabel = profile.plan_type === 'legacy_plus' ? 'Legacy Plus' :
    profile.plan_type === 'family_legacy' ? 'Family Legacy' :
    profile.plan_type === 'free_trial' ? 'Free Trial' : 'Free';
      ...
      <div className="px-4">
        ...
        <div className="mb-4">
          ...
          {['About Me', 'Location', 'Relationship', 'Work'].map(section => (
            <button
              key={section}
              onClick={() => navigate(createPageUrl('EditProfile') + `?section=${section.toLowerCase()}`)}
        ...
        <button
          ...
          className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-red-50 transition-colors mt-4"
        >
      ...
      <AlertDialog open={showLogout} onOpenChange={setShowLogout}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure you want to sign out of Lifescribe?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
