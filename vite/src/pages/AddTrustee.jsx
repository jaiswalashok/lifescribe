...
export default function AddTrustee() {
  ...
  const filteredConns = searchQ
    ? connections.filter(c => c.connected_user_name?.toLowerCase().includes(searchQ.toLowerCase()))
    : connections;
  ...
  const handleSave = async () => {
    ...
    setTimeout(() => navigate(createPageUrl('MemorialSettings')), 1200);
  };

  return (
    <div className="min-h-screen bg-white">
      <div className="flex items-center px-4 pt-12 pb-4">
        <button onClick={() => step > 1 ? setStep(step - 1) : navigate(-1)} className="text-gray-400">
          <ChevronLeft className="w-6 h-6" />
        </button>
        <h1 className="text-lg font-semibold text-[#111111] ml-3">
          Add Trustee — Step {step} of 3
        </h1>
      </div>
      ...
      <div className="px-6">
          ...
          <div>
            ...
            <div className="space-y-1 max-h-[50vh] overflow-y-auto">
              {filteredConns.map(conn => (
                <button
                  key={conn.id}
                  onClick={() => { setSelectedConn(conn); setStep(2); }}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-colors ${
                    selectedConn?.id === conn.id ? 'bg-[#1A1A2E] text-white' : 'hover:bg-gray-50'
                  }`}
                >
                  <MoodRingAvatar src={conn.connected_user_avatar} mood={conn.connected_user_mood} size={40} name={conn.connected_user_name} />
                  <div>
                    <p className={`text-sm font-medium ${selectedConn?.id === conn.id ? 'text-white' : 'text-[#111111]'}`}>
                      {conn.connected_user_name}
                    </p>
                    {conn.relationship_label && (
                      <p className={`text-xs ${selectedConn?.id === conn.id ? 'text-gray-300' : 'text-gray-400'}`}>
                        {conn.relationship_label}
                      </p>
                    )}
                  </div>
              ...
              {filteredConns.length === 0 && (
                <p className="text-sm text-gray-400 py-8 text-center">No connections found.</p>
              )}
            </div>
          </div>
        )}
        ...
        {step === 2 && selectedConn && (
          <div>
            ...
            <div className="space-y-2 mb-6">
              {[
                { value: 'full', label: 'Everything', desc: 'Full vault access' },
                { value: 'specific_chapters', label: 'Specific chapters only', desc: 'Choose which chapters' },
                { value: 'exclude_chapters', label: 'Everything except', desc: 'Exclude specific chapters' },
              ].map(opt => (
                <button
                  key={opt.value}
                  onClick={() => setPermissionsType(opt.value)}
                  className={`w-full p-4 rounded-xl border-2 text-left transition-all ${
                    permissionsType === opt.value ? 'border-[#1A1A2E] bg-[#FAFAFA]' : 'border-gray-100'
                  }`}
                >
                  ...
                  {chapters.map(ch => (
                    <button
                      ...
                      className={`w-full p-3 rounded-xl text-left text-sm transition-all ${
                        selectedChapterIds.includes(ch.id) ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-[#111111]'
                      }`}
                    >
                      {ch.name}
                    </button>
            ...
            <div className="flex items-center justify-between p-3 bg-[#F5F5F5] rounded-xl mb-6">
              ...
              <button
                onClick={() => setCanPassOn(!canPassOn)}
                className={`w-11 h-6 rounded-full transition-colors ${canPassOn ? 'bg-[#1A1A2E]' : 'bg-gray-200'}`}
              >
                <div className={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${canPassOn ? 'translate-x-5.5 ml-[22px]' : 'ml-0.5'}`} />
              </button>
            </div>
        ...
        {step === 3 && selectedConn && (
          <div>
            ...
            <div className="space-y-2 mb-6">
              {[
                { value: 'on_death', label: 'Upon confirmed death', desc: 'Default and most common' },
                { value: 'specific_date', label: 'On a specific date', desc: 'Choose a future date' },
                { value: 'milestone', label: 'On a recipient milestone', desc: 'Their 18th birthday, wedding, etc.' },
              ].map(opt => (
                <button
                  key={opt.value}
                  onClick={() => setDeliveryTrigger(opt.value)}
                  className={`w-full p-4 rounded-xl border-2 text-left transition-all ${
                    deliveryTrigger === opt.value ? 'border-[#1A1A2E] bg-[#FAFAFA]' : 'border-gray-100'
                  }`}
                >
                  <p className="text-sm font-medium text-[#111111]">{opt.label}</p>
                  <p className="text-xs text-gray-400">{opt.desc}</p>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-1">
                  {['Their 18th birthday', 'Their wedding day', 'One year after my death'].map(opt => (
                    <button
                      key={opt}
                      onClick={() => setDeliveryDetail(opt)}
                      className={`w-full p-3 rounded-xl text-left text-sm transition-all ${
                        deliveryDetail === opt ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-[#111111]'
                      }`}
                    >
                      {opt}
                    </button>
