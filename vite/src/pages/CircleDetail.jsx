...
export default function CircleDetail() {
  ...
  const isUnclassified = urlParams.get('type') === 'unclassified';
  ...
  const { data: allRelationships = [] } = useQuery({
    ...
    queryFn: () => base44.entities.FamilyRelationship.list(),
  });

  const circle = isUnclassified ? { name: 'Unclassified', circle_type: 'custom' } : circles.find(c => c.id === circleId);
  const members = isUnclassified
    ? connections.filter(c => !c.circle_id)
    : connections.filter(c => c.circle_id === circleId);

  const filteredMembers = searchQuery
    ? members.filter(m => m.connected_user_name?.toLowerCase().includes(searchQuery.toLowerCase()))
    : members;
  ...
  // Build enriched connections including inferred relatives
    ...
    for (const [userId, info] of inferredMap.entries()) {
      if (!directIds.has(userId) && info.isInferred) {
        result.push({
          ...
          connected_user_name: info.name,
          connected_user_avatar: info.avatar,
          connected_user_mood: info.mood,
          relationship_label: info.relationship,
        });
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={handleEditOpen}>
                  <Pencil className="w-4 h-4 mr-2" /> Rename circle
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleTogglePrivacy}>
                  {circle?.privacy_setting === 'private' ? (
                    <>
                      <Globe className="w-4 h-4 mr-2" /> Make visible to connections
                    </>
                  ) : (
                    <>
                ...
                {!isFamily && (
                  <DropdownMenuItem onClick={handleDeleteCircle} className="text-red-500 focus:text-red-500">
                    <Trash2 className="w-4 h-4 mr-2" /> Delete circle
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        {isFamily ? (
          <>
            <div className="mb-4 p-3 bg-white rounded-xl flex items-center justify-between text-sm">
              <span className="text-gray-600">
                {circle?.privacy_setting === 'private' ? '🔒 Private' : '👥 Visible to connections'}
              </span>
              <button
                onClick={handleTogglePrivacy}
                className="text-[#111111] font-medium hover:text-gray-600 transition-colors"
              >
                Change
              </button>
            </div>
            <FamilyTree
              connections={enrichedFamilyConnections}
              selfProfile={selfProfile}
              onTapNode={(conn) => { setSelectedConn(conn); setShowNodeSheet(true); }}
              onAddNode={() => setShowFamilyAddSheet(true)}
            />
            <Button
              onClick={() => setShowFamilyAddSheet(true)}
          ...
          <>
            {filteredMembers.length === 0 ? (
              <EmptyState
                message="No members yet. Invite someone to this circle."
              ...
              <div className="space-y-1">
                {filteredMembers.map(conn => (
                  <button
                    key={conn.id}
                    onClick={() => { setSelectedConn(conn); setShowNodeSheet(true); }}
                    className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white transition-colors text-left"
                  >
                    <MoodRingAvatar
                      src={conn.connected_user_avatar}
                      mood={conn.connected_user_mood}
                      size={40}
      ...
      <AnimatePresence>
        {showNodeSheet && selectedConn && (
          <ConnectionProfile connection={selectedConn} onClose={() => setShowNodeSheet(false)} onDelete={() => handleDeleteConnection(selectedConn.id)} />
        )}
      </AnimatePresence>
      ...
      <AddFriendsSheet
        ...
        circleId={circleId}
        onAdded={() => queryClient.invalidateQueries({ queryKey: ['connections'] })}
      />

      {/* Edit circle name sheet */}
      ...
      <AddFamilyMemberSheet
        open={showFamilyAddSheet}
        onClose={() => setShowFamilyAddSheet(false)}
        circleId={circleId}
        onAdded={() => queryClient.invalidateQueries({ queryKey: ['connections'] })}
      />
    </div>
