    ...
    const initProfile = async () => {
      try {
        ...
        if (profiles.length === 0) {
          ...
          const data = signupData ? JSON.parse(signupData) : {};
          await base44.entities.UserProfile.create({
            user_id: 'current',
            full_name: user.full_name || '',
            username: data.username || user.email?.split('@')[0] || '',
            date_of_birth: data.date_of_birth || '',
            profile_picture_url: data.profile_picture_url || user.profile_picture || '',
            plan_type: localStorage.getItem('lifescribe_plan') || 'free_trial',
            trial_start_date: new Date().toISOString().split('T')[0],
            language: localStorage.getItem('lifescribe_lang') || 'en',
          });
          if (signupData) localStorage.removeItem('lifescribe_signup');
        }
      } catch {
        // not signed in yet
      }
      ...
      <div className="bg-white/80 backdrop-blur-sm px-6 pt-14 pb-4 border-b border-gray-100/50">
        ...
        <div className="flex gap-0 bg-[#F5F5F5] rounded-full p-1">
          <button
            ...
            className={`flex-1 py-2 text-sm font-medium rounded-full transition-all ${activeTab === 'journals' ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'}`}
          >
            My Journals
          </button>
          <button
            onClick={() => setActiveTab('world')}
            className={`flex-1 py-2 text-sm font-medium rounded-full transition-all ${activeTab === 'world' ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'}`}
          >
            My World
          </button>
        </div>
