...
const INDIVIDUAL_PLANS = [
  { storage: '50GB', prices: { developed: { monthly: 10, annual: 100 }, emerging: { monthly: 5, annual: 50 }, lowest: { monthly: 3, annual: 30 } } },
  { storage: '200GB', prices: { developed: { monthly: 15, annual: 150 }, emerging: { monthly: 8, annual: 80 }, lowest: { monthly: 5, annual: 50 } } },
  { storage: '500GB', prices: { developed: { monthly: 20, annual: 200 }, emerging: { monthly: 10, annual: 100 }, lowest: { monthly: 7, annual: 70 } } },
  { storage: '1TB', prices: { developed: { monthly: 30, annual: 300 }, emerging: { monthly: 15, annual: 150 }, lowest: { monthly: 10, annual: 100 } } },
];
...
const FEATURES = [
  'Unlimited entries (limited by storage only)', 'Unlimited LifeCircles', 'Unlimited LifeTrustees',
  'Unlimited LifeChapters', 'Unlimited MomentCapsules', 'Multi-device sync',
  'End-to-end encryption', 'Full data export', 'No ads, ever', 'Data never sold',
];
...
export default function Paywall() {
  ...
  if (planType === 'individual') {
    ...
    if (plan) displayPrice = plan.prices[region][billing];
  } else {
    const plan = FAMILY_PLANS.find(p => p.storage === selectedFamilyStorage);
    if (plan) displayPrice = billing === 'monthly' ? plan.monthly : plan.annual;
  }

  return (
    <div className="min-h-screen bg-white flex flex-col px-5 pt-12 pb-10 max-w-md mx-auto">
      <button onClick={() => router.back()} className="flex items-center gap-2 text-[#111111] mb-6 -ml-1">
      ...
      <div className="flex bg-[#F5F5F5] rounded-full p-1 mb-6">
        {['individual', 'family'].map(type => (
          <button key={type} onClick={() => setPlanType(type)}
            className={`flex-1 py-2 text-sm font-medium rounded-full transition-all ${planType === type ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'}`}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex bg-[#F5F5F5] rounded-full p-1 gap-1">
          <button onClick={() => setBilling('monthly')}
            className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all ${billing === 'monthly' ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'}`}>
            Monthly
          </button>
          <button onClick={() => setBilling('annual')}
            className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all ${billing === 'annual' ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'}`}>
            Annual <span className="ml-1 text-[10px] text-green-600 font-semibold">-17%</span>
          </button>
        </div>
        {billing === 'annual' && <span className="text-xs text-green-600 font-medium">2 months free</span>}
      </div>
        ...
        <>
            ...
            <div className="flex gap-1 overflow-x-auto pb-1">
              {REGIONS.map(r => (
                <button key={r.id} onClick={() => setRegion(r.id)}
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${region === r.id ? 'bg-[#111111] text-white' : 'bg-[#F5F5F5] text-gray-500'}`}>
                  {r.label.split(' ')[0]}
                </button>
              ))}
            </div>
            <button onClick={() => setShowRegionNote(!showRegionNote)} className="flex items-center gap-1 text-[10px] text-gray-400 mt-1.5">
              {REGIONS.find(r => r.id === region)?.label}
              {showRegionNote ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            </button>
            {showRegionNote && <p className="text-[10px] text-gray-400 mt-1 leading-relaxed">{selectedRegionNote}</p>}
          </div>
          <div className="space-y-2.5 mb-6">
            {INDIVIDUAL_PLANS.map(plan => {
              ...
              return (
                <button key={plan.storage} onClick={() => setSelectedStorage(plan.storage)}
                  className={`w-full p-4 rounded-2xl border-2 text-left transition-all ${isSelected ? 'border-[#111111] bg-[#FAFAFA]' : 'border-gray-100 bg-white'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-[#111111]' : 'border-gray-300'}`}>
        ...
        <>
          ...
          <div className="space-y-2.5 mb-6">
            {FAMILY_PLANS.map(plan => {
              ...
              return (
                <button key={plan.storage} onClick={() => setSelectedFamilyStorage(plan.storage)}
                  className={`w-full p-4 rounded-2xl border-2 text-left transition-all ${isSelected ? 'border-[#111111] bg-[#FAFAFA]' : 'border-gray-100 bg-white'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-[#111111]' : 'border-gray-300'}`}>
                    ...
                    <div className="text-right">
                      <p className="font-bold text-[#111111] text-sm">${price}</p>
                      <p className="text-[10px] text-gray-400">{billing === 'monthly' ? '/month' : '/year'}</p>
                      {billing === 'annual' && <p className="text-[10px] text-green-600">Save ${plan.savings}</p>}
                    </div>
                  </div>
                </button>
              );
      ...
      <button onClick={() => setShowFeatures(!showFeatures)} className="flex items-center justify-between w-full mb-3 text-sm font-medium text-[#111111]">
      ...
      <Button onClick={handleStartTrial} className="w-full bg-[#111111] text-white hover:bg-[#333] rounded-full h-12 text-sm font-semibold mb-3">
        Start 3 months free
      </Button>
      <Button onClick={handleSubscribe} variant="outline" className="w-full border-gray-200 text-[#111111] hover:bg-gray-50 rounded-full h-12 text-sm font-medium">
        Subscribe now · ${displayPrice}/{billing === 'monthly' ? 'mo' : 'yr'}
      </Button>

      <p className="text-center text-[10px] text-gray-400 mt-4">No ads · No data selling · You own 100% of your content</p>
