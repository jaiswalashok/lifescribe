...
export default function Milestones() {
  ...
  const sorted = [...milestones].sort((a, b) => {
    const aDate = new Date(a.milestone_date), bDate = new Date(b.milestone_date), now = new Date();
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <div className="px-4 pt-4 pb-12">
        {sorted.length === 0 ? (
          <EmptyState icon={Star} message="No milestones yet. Add the moments that matter." ctaText="Add milestone" onAction={() => router.push(createPageUrl('AddMilestone'))} />
        ) : (
          <>
            <div className="space-y-3 mb-6">
              {sorted.map(ms => {
                ...
                return (
                  <button key={ms.id} onClick={() => router.push(createPageUrl('MilestoneDetail') + `?id=${ms.id}`)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{ms.milestone_date ? format(new Date(ms.milestone_date), 'MMM d, yyyy') : ''}</p>
                    </div>
                    <span className="text-xs bg-[#F5F5F5] text-gray-500 px-2.5 py-1 rounded-full font-medium">{daysUntil === 0 ? 'Today!' : `${daysUntil}d`}</span>
                  </button>
                );
              })}
            </div>
            <Button onClick={() => router.push(createPageUrl('AddMilestone'))} className="w-full bg-[#111111] text-white hover:bg-[#333] rounded-full h-12 text-sm font-medium">
              Add milestone
            </Button>
          </>
        )}
