...
function Toggle({ value, onChange }) {
  return (
    <button
      ...
      className={`w-11 h-6 rounded-full transition-colors flex-shrink-0 ${value ? 'bg-[#1A1A2E]' : 'bg-gray-200'}`}
    >
...
const UPCOMING_DAYS_KEY = 'circles_upcoming_days';
...
export default function NotificationSettings() {
  ...
  const [settings, setSettings] = useState({
    ...
    connection_posts: true,
    payment_reminders: true,
  });
  const [upcomingDays, setUpcomingDays] = useState(() => {
    return parseInt(localStorage.getItem(UPCOMING_DAYS_KEY) || '30', 10);
  });
  const [toast, setToast] = useState('');

  const handleUpcomingDays = (days) => {
    setUpcomingDays(days);
    localStorage.setItem(UPCOMING_DAYS_KEY, String(days));
  };
        ...
        <div className="pt-2 pb-1">
          ...
          <div className="flex gap-2">
            {[7, 14, 30].map(days => (
              <button
                key={days}
                onClick={() => handleUpcomingDays(days)}
                className={`flex-1 py-2 rounded-full text-xs font-medium transition-all ${
                  upcomingDays === days ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-gray-500'
                }`}
              >
                {days} days
              </button>
