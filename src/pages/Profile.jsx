...
export default function Profile() {
  ...
  const { data: capsules = [] } = useQuery({
    ...
    queryFn: () => base44.entities.MomentCapsule.list('-created_date'),
  });
    ...
    <div className="min-h-screen bg-[#FAFAFA] pb-24">
      ...
      <div className="bg-white px-6 pt-14 pb-6">
        ...
        {profile.current_mood && (
          <PillBadge text={getMoodLabel(profile.current_mood)} className="mb-4" />
        )}
        ...
        <div className="space-y-2 mb-4">
          {profile.relationship_status && (
            <div className="flex items-center gap-2 text-xs text-gray-500">
              ...
              {profile.relationship_partner_name && profile.display_partner_name && (
                <span>with {profile.relationship_partner_name}</span>
              )}
            </div>
          )}
        ...
        {/* Milestones shortcut */}
        <Button
          onClick={() => navigate(createPageUrl('Milestones'))}
          variant="outline"
          size="sm"
          ...
          {['journals', 'shared', 'chapters'].map(tab => (
            <button
              ...
              className={`flex-1 py-2 text-xs font-medium rounded-full transition-all ${
                activeTab === tab ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'
              }`}
            >
      ...
      <div className="px-4 pt-4">
        ...
        {activeTab === 'shared' && (
          <div className="space-y-3">
            {capsules.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-12">No shared memories yet.</p>
            )}
            {capsules.map(cap => (
              <button
                key={cap.id}
                onClick={() => navigate(createPageUrl('CapsuleDetail') + `?id=${cap.id}`)}
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    {cap.event_date && (
                      <span className="text-xs text-gray-400 flex items-center gap-1">
                        ...
                        {format(new Date(cap.event_date), 'MMM d, yyyy')}
                      </span>
                    )}
        ...
        {activeTab === 'chapters' && (
          <div className="space-y-3">
            {chapters.map(ch => (
              <button
                key={ch.id}
                onClick={() => navigate(createPageUrl('ChapterDetail') + `?id=${ch.id}`)}
