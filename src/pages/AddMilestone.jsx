...
export default function AddMilestone() {
  ...
  const handleSave = async () => {
    ...
    await base44.entities.Milestone.create({
      ...
      reminder_days_before: reminders.length > 0 ? reminders : undefined,
    });
    queryClient.invalidateQueries({ queryKey: ['milestones'] });
    setToast('Milestone saved.');
    setTimeout(() => navigate(createPageUrl('Milestones')), 1200);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="px-6 space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {MILESTONE_TYPES.map(mt => {
              const Icon = TYPE_ICONS[mt.value];
              return (
                <button
                  key={mt.value}
                  onClick={() => setType(mt.value)}
                  className={`p-4 rounded-xl flex flex-col items-center gap-2 transition-all ${
                    type === mt.value ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-[#111111]'
                  }`}
                >
        ...
        <Button
          onClick={handleSave}
          disabled={!title.trim() || !milestoneDate || !type || saving}
