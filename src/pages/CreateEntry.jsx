    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 px-6">
        <textarea
          ...
          className="w-full h-full min-h-[300px] text-base text-[#111111] placeholder:text-gray-300 
                     resize-none border-0 outline-none leading-relaxed bg-transparent"
        />
      </div>
          ...
          {['private', 'connections'].map(a => (
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded-full transition-all ${
                audience === a ? 'bg-white text-[#111111] shadow-sm' : 'text-gray-400'
              }`}
            >
      ...
      <BottomSheet open={showMood} onClose={() => setShowMood(false)} title="What mood are you in today?">
        <div className="grid grid-cols-3 gap-2">
          {MOOD_OPTIONS.map(opt => (
            <button
              key={opt.value}
              onClick={() => { setMood(opt.value); setShowMood(false); setShowSleep(true); }}
              className={`p-3 rounded-xl text-center transition-all ${
                mood === opt.value ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-[#111111] hover:bg-gray-100'
              }`}
            >
      ...
      <BottomSheet open={showSleep} onClose={() => setShowSleep(false)} title="How did you sleep?">
        <div className="space-y-2">
          {SLEEP_OPTIONS.map(opt => (
            <button
              key={opt.value}
              onClick={() => { setSleepQuality(opt.value); setShowSleep(false); setShowMotivation(true); }}
              className={`w-full p-3.5 rounded-xl text-left flex items-center gap-3 transition-all ${
                sleepQuality === opt.value ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-[#111111]'
              }`}
            >
      ...
      <BottomSheet open={showMotivation} onClose={() => setShowMotivation(false)} title="How motivated are you?">
        <div className="space-y-2">
          {MOTIVATION_OPTIONS.map(opt => (
            <button
              key={opt.value}
              onClick={() => { setMotivation(opt.value); setShowMotivation(false); }}
              className={`w-full p-3.5 rounded-xl text-left flex items-center gap-3 transition-all ${
                motivation === opt.value ? 'bg-[#1A1A2E] text-white' : 'bg-[#F5F5F5] text-[#111111]'
              }`}
            >
              <span className="text-lg">{opt.emoji}</span>
              <span className="text-sm font-medium">{opt.label}</span>
