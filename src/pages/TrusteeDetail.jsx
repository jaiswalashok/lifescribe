...
export default function TrusteeDetail() {
  ...
  const trusteeId = urlParams.get('id');
  ...
  const handleDelete = async () => {
    ...
    setTimeout(() => navigate(createPageUrl('MemorialSettings')), 1000);
  };

  const permissionsLabel = trustee.permissions_type === 'full'
    ? 'Everything — full vault access'
    : trustee.permissions_type === 'specific_chapters'
    ? 'Specific chapters only'
    : 'Everything except selected chapters';

  const deliveryLabel = trustee.delivery_trigger === 'on_death'
    ? 'Upon confirmed death'
    : trustee.delivery_trigger === 'specific_date'
    ? `On ${trustee.delivery_detail}`
    : `On milestone: ${trustee.delivery_detail}`;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-6">
        ...
        <Button
          ...
          variant="outline"
          className="w-full rounded-full h-12 text-sm border-red-200 text-red-500 hover:bg-red-50"
        >
      ...
      <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove this trustee?</AlertDialogTitle>
            <AlertDialogDescription>They will no longer inherit your vault.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
